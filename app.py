...
class student(db.Model):
    id = db.Column(
        db.Integer, db.Sequence("seq_reg_id", start=1, increment=1), primary_key=True
    )
...
def result():
    if request.method == "POST":
        ...
        db.session.commit()
        if int(result) == 1:
            prediction = "Prediction shows that you need to consult a doctor"
        else:
...
# Define a static list of doctors (dummy data)
doctors = [
    {
        "id": 1,
        "name": "Dr. John Smith",
